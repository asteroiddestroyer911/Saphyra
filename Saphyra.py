...
def httpcall(url):
    ...
    try:
        ...
        inc_counter()
        time.sleep(random.uniform(3, 7))  # Added random delay to avoid detection
    except urllib.error.HTTPError as e:
        if e.code == 429:
            print("Too Many Requests - Adding a delay...")
            time.sleep(random.uniform(7, 12))  # Added longer wait to prevent bans
        else:
            set_flag(1)
...
if len(sys.argv) < 2:
    ...
    threads = [HTTPThread() for _ in range(500)]
